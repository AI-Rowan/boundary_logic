...
| `.claude/reference/` | Generic (non-script-specific) code walkthrough docs and archived implementation plans (after approval + execution). Tracked by git. | When a plan from `.claude/plans/` has been implemented, copy it here with an IMPLEMENTED banner. |
| `development/` | The main development/testing harness (`03_loan_status_Boundary_Logic.R`) and its per-script code walkthroughs (`development/reference/`). Tracked by git; excluded from the built package via `.Rbuildignore`. | When a script-specific walkthrough is written for the harness script, add it to `development/reference/`. |
...
### Script-specific references (tied to `development/03_loan_status_Boundary_Logic.R`)
...
|---|---|---|
| `development/reference/review_section4_to_6.md` | Steps 3–6 (Phase 1) | `data_prepare.R`, `outlier_filter.R`, `model_fit.R`, `model_utils.R`, `result.R`, `biplot_grid.R`, `projection.R`, `plot_biplot.R`, `project_points.R` |
| `development/reference/review_section7_to_8.md` | Step 7 (Phase 2) | `boundary.R`, `boundary_plot.R`, `hull_utils.R`, `feasibility_utils.R` |
| `development/reference/review_section9_to_15.md` | Steps 9–15 (Phase 3) | `local_cf.R`, `shapley.R`, `project_points.R` |

Script: `development/03_loan_status_Boundary_Logic.R`
...
| `data_prepare.R` | `bl_prepare_data()`, `bl_wrap_data()`, `bl_set_scaling()` (records raw-unit display scaling) |
...
| `plot_biplot.R` | `plot_biplotEZ()`, `plot.bl_result()` — biplot rendering; private helpers `.make_label_line_vec()`, `.make_ticks_vec()`, `.apply_biplot_rotation()`, and `.bl_rescale_biplot_axes()` (raw-unit axis relabel) |
...
- **Every biplot plot method must accept `new_title = NA`** for a custom title, applied as `if (!is.na(new_title)) <biplot_obj>$Title <- new_title` after the biplotEZ object is fetched (and after any auto-generated title is assigned, so the user override wins). biplotEZ renders `$Title` via `graphics::title()` at plot-flush time. Do not validate `new_title` (none of the plot methods do); `title()` coerces most inputs via `as.character()` -- only non-coercible types (a function/closure or environment) error, and they error from inside the biplotEZ `plot()` flush, not from arg handling. Implemented uniformly across `plot_biplotEZ()`, `plot.bl_result()` (via `...`), `plot.bl_local_result()`, `plot.bl_surrogate()`, and `plot.bl_sparse_result()` (via `...`).
- **Raw-unit biplot axes.** When `bl_result$scaling` is present (set via `bl_set_scaling()` when the user standardised features before model fitting), every biplot plot method must relabel its axes into original units by calling `.bl_rescale_biplot_axes(<biplot_obj>, bl_result$scaling, var_names)` **after** any rotation and `new_title` override, **before** the first `plot()`. The helper transforms only `$means`/`$sd` (and forces `$scaled`/`$center` TRUE); it never touches geometry (`$Z`, `$Lmat`, `$ax.one.unit`), so plotted positions are byte-identical. It is a no-op when `scaling` is `NULL` (back-compatible with older `bl_result` objects). Wired into `plot_biplotEZ()`, `plot.bl_projection()`, `plot.bl_local_result()`, `plot.bl_surrogate()`, and (via pass-through) `plot.bl_sparse_result()`. The scaling is **display-only** — it never alters the projection, grid, or what the model/`predict_fn` sees. Distinct from `X_center`/`X_sd` (internal PCA standardisation) and the `standardise` flag.
- **Raw-unit feature *values* (display-only).** Any plot/print that shows feature values (observed, counterfactual, sparse CF, target) must convert them to original units via `.scale_to_raw(values, bl_result$scaling, var_names, kind)` (`R/data_prepare.R`) when `scaling` is present. **The `kind` argument is the correctness rule:** a *level* (an observed/CF/sparse value) maps `value*scale + center` (`kind = "level"`); a *delta* (a difference such as the Shapley `data_to_boundary` change) maps `value*scale` with **no** centre term (`kind = "delta"`). No-op when `scaling` is `NULL`; warns and leaves values unchanged if it doesn't cover every feature. The helper strips names — re-attach `names()` after calling it if the display needs them. Stored objects stay in model units (convert only at display). Do **not** convert quantities that are not in feature units — e.g. Shapley *contributions* (`shapley_cause`) are prediction-impact units and must be left as-is. Currently applied in `plot.bl_shapley()`, `print.bl_shapley()`, `print.bl_sparse_result()`, `print.bl_target()`, and `print.bl_counterfactual()`.
- **Counterfactual objects mirror `bl_target`.** `bl_find_local_cf()` returns `bl_local$bl_counterfactual` (class `"bl_counterfactual"`, via `.make_bl_counterfactual()`) parallel to `bl_local$bl_target`: it stores the boundary point's values in **model units** (`x_cf = B_x`) plus `scaling`, and its `print` method converts to raw at display time. When adding a new "point" companion object, follow this pattern (store model-unit values + `scaling`; convert in `print` via `.scale_to_raw`).
- **In scripts/vignettes, fit standardisation on the training split only.** When demonstrating `bl_set_scaling()`, split first (`bl_prepare_data()` on raw data — the hull filter self-standardises, so filtering on raw vs standardised is equivalent), compute `center`/`scale` from `bl_dat$train_data`, apply those stats to both `train_data` and `test_data`, then record with `bl_set_scaling()`. Computing the scaling on the full dataset leaks test information. See `development/03_loan_status_Boundary_Logic.R` v2 block.
...
- **Before any bulk `rm`/`unlink` of "scratch" files, diff the list against the `git status` snapshot taken at session start.** Never assume an untracked file is yours to delete -- untracked files have no git history and `rm` on Windows does not use the Recycle Bin. (Followed a real loss of user files -- `Rplots.pdf`, `verify_labels.R`, etc. -- on 2026-06-08.)
...
- **Variable-specific standardisation methods.** `bl_set_scaling()` currently records a single affine method (one `method` label) applied across all features, though each feature carries its own `(center, scale)`. A future enhancement would allow a *different transform family per feature* (z-score for some, min-max for others, log/Box-Cox for skewed features, "none" for already-interpretable ones). This needs `scaling` generalised from two numeric vectors + one method label to a per-variable transform spec holding each feature's forward and inverse function. Caveat: non-affine inverses (e.g. log) produce non-linearly spaced axis ticks, and the biplotEZ linear axis cannot render those exactly — such transforms would need biplotEZ's spline-calibrated axis path (`PCOaxes = "splines"`) or a documented "affine-only exact" limitation. Keep the current uniform-affine `bl_set_scaling()` as the simple default.
- ~~**Raw-unit reporting of counterfactual/target values**~~ — **IMPLEMENTED 2026-06-16.** Observed/counterfactual/sparse feature values now display in raw units when `bl_result$scaling` is set, via the private `.scale_to_raw(values, scaling, var_names, kind)` helper (`R/data_prepare.R`; `kind = "level"` -> `value*scale + center`, `kind = "delta"` -> `value*scale`). Applied at display time in `plot.bl_shapley()` (label becomes observed -> counterfactual), `print.bl_shapley()`, `print.bl_sparse_result()`, and `print.bl_target()` (`bl_select_target()` now stores `scaling` on the `bl_target`). `bl_find_local_cf()` also returns a `bl_counterfactual` companion object (`bl_local$bl_counterfactual`, class `"bl_counterfactual"`, built by `.make_bl_counterfactual()`) that mirrors `bl_target` and prints the boundary counterfactual in raw units; `print.bl_local_result()` shows Target and Counterfactual blocks together. Stored objects stay in model units. Shapley *contributions* (`shapley_cause`) are prediction-impact units, not feature units, and are intentionally left unconverted.
- **`.bl_rotate()` local biplot plane is under-determined** (found 2026-07-15; **documented, deliberately not actioned**). `Y <- rbind(-x, 0, x)` is rank 1, so the Procrustes cross-product `M = t(YV) %*% YVr_padded` is rank 1 with a `p-1`-dimensional exactly degenerate null space. The minimiser is pinned only along the target direction; the plane's second direction is LAPACK's arbitrary tie-break. Measured: bases differing by 4.6e-15 give planes 65.9 deg apart; only 46% of the plane's second direction lies in the `proj_pair` eigenplane it is named after; the target's own coordinates are identical regardless. **Inherited from the original PhD method** (`origin/original_PhD_code:scripts/1.3 Optimal Rotation.R`) -- the package is a faithful port, not a porting bug. Full derivation, measurements, and the minimal-rotation option considered: `documentation/rotation_plane_underdetermination_note.md`. Do not "fix" this without an explicit instruction: it moves every Phase 3 counterfactual and so is not covered by the S4 "outputs proven equivalent" rule.
...
- **Shapley plot decimal-place consistency.** `plot.bl_shapley()` labels (`"VarName: observed -> counterfactual"`) currently round the observed value to 3 d.p. and the counterfactual/change value to 2 d.p. (changed 2026-06-17 per explicit request, in both the default model-units path in `bl_shapley()` and the raw-units override in `plot.bl_shapley()` — see `R/shapley.R`). Review whether observed and counterfactual should share one decimal-place convention, and whether label spacing/alignment needs adjustment now that the two numbers can have different precision.
